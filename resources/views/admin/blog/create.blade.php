...
@section('title', 'Thêm bài viết - Tâm Barbershop Admin')
...
<div class="max-w-5xl mx-auto space-y-8 py-4">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-6">
            <div>
                <div class="flex items-center gap-3 mb-2">
                    ...
                    <div>
                        <span class="text-[10px] font-bold uppercase tracking-[0.2em] text-[#a8342f]">Góc chia sẻ / Nội dung</span>
                ...
                <p class="text-xs text-[#f4ecd8]/70">
                    Tạo mới bài viết kinh nghiệm, tư vấn tạo kiểu và tin tức barbershop để xuất bản lên website.
                </p>
            </div>
    ...
    <div class="rounded-[4px] border border-[#3c2c15] bg-[#110d07] p-6 sm:p-8 shadow-2xl">
        <form action="{{ route('admin.blog.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6" id="blog-create-form">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="md:col-span-2 space-y-1.5">
                    ...
                    <input id="title" type="text" name="title" value="{{ old('title') }}" required 
                           ...
                           placeholder="VD: 5 kiểu tóc đẹp nâng tầm phong cách cho mùa hè">
                ...
                <div class="space-y-1.5">
                    ...
                    <input id="category" type="text" name="category" value="{{ old('category') }}" 
                           class="w-full rounded-[2px] border border-[#3c2c15] bg-[#070503] px-4 py-2.5 text-sm text-[#f4ecd8] focus:border-[#8a641d] focus:outline-none focus:ring-1 focus:ring-[#8a641d] transition-all placeholder-[#6f6248]" 
                           placeholder="VD: Kiến thức, Xu hướng, Chăm sóc tóc...">
                </div>
                ...
                <div class="md:col-span-2 space-y-1.5">
                    ...
                    <textarea id="excerpt" name="excerpt" rows="3" 
                              class="w-full rounded-[2px] border border-[#3c2c15] bg-[#070503] px-4 py-2.5 text-sm text-[#f4ecd8] focus:border-[#8a641d] focus:outline-none focus:ring-1 focus:ring-[#8a641d] transition-all placeholder-[#6f6248]" 
                              placeholder="Giới thiệu ngắn gọn gây tò mò cho bài viết...">{{ old('excerpt') }}</textarea>
                </div>
                ...
                <div class="md:col-span-2 space-y-1.5">
                    ...
                    <textarea id="content" name="content" rows="12" required 
                              class="w-full rounded-[2px] border border-[#3c2c15] bg-[#070503] px-4 py-2.5 text-sm text-[#f4ecd8] focus:border-[#8a641d] focus:outline-none focus:ring-1 focus:ring-[#8a641d] transition-all placeholder-[#6f6248]" 
                              placeholder="Nhập nội dung đầy đủ của bài viết...">{{ old('content') }}</textarea>
                ...
                <div class="space-y-1.5">
                    ...
                    <label for="thumbnail" 
                        ...
                        <img id="thumbnail-preview" class="hidden max-h-48 w-full rounded-[2px] object-cover" alt="preview">
                        <span id="thumbnail-placeholder" class="flex flex-col items-center text-[#6f6248] group-hover:text-[#f2d788] transition-colors">
                ...
                <div class="space-y-6">
                    <div class="space-y-1.5">
                        ...
                        <select id="status" name="status" class="w-full rounded-[2px] border border-[#3c2c15] bg-[#070503] px-4 py-2.5 text-sm text-[#f4ecd8] focus:border-[#8a641d] focus:outline-none focus:ring-1 focus:ring-[#8a641d] transition-all">
                            <option value="draft" {{ old('status') === 'draft' ? 'selected' : '' }} class="bg-[#070503] text-[#f4ecd8]">Bản nháp</option>
                            <option value="scheduled" {{ old('status') === 'scheduled' ? 'selected' : '' }} class="bg-[#070503] text-[#f4ecd8]">Chờ xuất bản</option>
                            <option value="published" {{ old('status', 'published') === 'published' ? 'selected' : '' }} class="bg-[#070503] text-[#f4ecd8]">Xuất bản ngay</option>
                        </select>
                    ...
                    <div class="space-y-1.5">
                        ...
                        <input id="publish_at" type="datetime-local" name="publish_at" value="{{ old('publish_at') }}" 
...
if (blogForm && blogSubmitBtn) {
    blogForm.addEventListener('submit', function () {
        ...
        blogSubmitBtn.innerHTML = '<i class="fa-solid fa-spinner fa-spin text-xs"></i> <span>Đang lưu...</span>';
    });
}
</script>
@endsection
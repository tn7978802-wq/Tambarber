...
<div class="max-w-4xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
    <div class="mb-8 flex items-center justify-between gap-4 border-b border-[#3c2c15] pb-5">
        <div>
            <p class="text-[11px] font-bold uppercase tracking-[0.2em] text-[#6f6248]">Nội dung</p>
            <h1 class="font-['Bebas_Neue'] text-4xl tracking-wider text-[#f2d788] uppercase">Thêm bài viết mới</h1>
        </div>
    ...
    <form action="{{ route('admin.blog.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6 rounded-[2px] border border-[#3c2c15] bg-[#171008] p-6 shadow-xl">
        @csrf
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div class="md:col-span-2">
                ...
                <input id="title" type="text" name="title" value="{{ old('title') }}" required class="w-full rounded-[2px] border border-[#3c2c15] bg-[#0b0805] px-4 py-2.5 text-sm text-[#f4ecd8] focus:border-[#8a641d] focus:outline-none focus:ring-1 focus:ring-[#8a641d]" placeholder="VD: 5 kiểu tóc đẹp cho mùa hè">
                @error('title') <p class="mt-1 text-xs text-red-400">{{ $message }}</p> @enderror
            </div>

            <div>
                <label for="slug" class="mb-2 block text-xs font-bold uppercase tracking-wider text-[#f4ecd8]/80">Slug</label>
                <input id="slug" type="text" name="slug" value="{{ old('slug') }}" class="w-full rounded-[2px] border border-[#3c2c15] bg-[#0b0805] px-4 py-2.5 text-sm text-[#f4ecd8] focus:border-[#8a641d] focus:outline-none focus:ring-1 focus:ring-[#8a641d]" placeholder="tu-dong-neu-de-trong">
            ...
            <div>
                ...
                <input id="category" type="text" name="category" value="{{ old('category') }}" class="w-full rounded-[2px] border border-[#3c2c15] bg-[#0b0805] px-4 py-2.5 text-sm text-[#f4ecd8] focus:border-[#8a641d] focus:outline-none focus:ring-1 focus:ring-[#8a641d]" placeholder="Kiến thức, xu hướng, chăm sóc...">
            </div>

            <div class="md:col-span-2">
                <label for="excerpt" class="mb-2 block text-xs font-bold uppercase tracking-wider text-[#f4ecd8]/80">Mô tả ngắn</label>
                <textarea id="excerpt" name="excerpt" rows="3" class="w-full rounded-[2px] border border-[#3c2c15] bg-[#0b0805] px-4 py-2.5 text-sm text-[#f4ecd8] focus:border-[#8a641d] focus:outline-none focus:ring-1 focus:ring-[#8a641d]" placeholder="Giới thiệu ngắn gọn về bài viết...">{{ old('excerpt') }}</textarea>
            </div>
            ...
            <div class="md:col-span-2">
                ...
                <textarea id="content" name="content" rows="12" required class="w-full rounded-[2px] border border-[#3c2c15] bg-[#0b0805] px-4 py-2.5 text-sm text-[#f4ecd8] focus:border-[#8a641d] focus:outline-none focus:ring-1 focus:ring-[#8a641d]" placeholder="Nhập nội dung bài viết...">{{ old('content') }}</textarea>
            ...
            <div>
                ...
                <label for="thumbnail" class="flex min-h-[180px] cursor-pointer flex-col items-center justify-center rounded-[2px] border-2 border-dashed border-[#3c2c15] bg-[#0b0805] p-4 text-center transition hover:border-[#8a641d]">
                    <img id="thumbnail-preview" class="hidden max-h-44 w-full rounded-[2px] object-cover" alt="preview">
                    <span id="thumbnail-placeholder" class="flex flex-col items-center text-[#6f6248]">
            ...
            <div>
                ...
                <select id="status" name="status" class="w-full rounded-[2px] border border-[#3c2c15] bg-[#0b0805] px-4 py-2.5 text-sm text-[#f4ecd8] focus:border-[#8a641d] focus:outline-none focus:ring-1 focus:ring-[#8a641d]">
                    ...
                    <option value="scheduled" {{ old('status') === 'scheduled' ? 'selected' : '' }}>Chờ xuất bản</option>
            ...
            <div class="md:col-span-2">
                ...
                <input id="publish_at" type="datetime-local" name="publish_at" value="{{ old('publish_at') }}" class="w-full rounded-[2px] border border-[#3c2c15] bg-[#0b0805] px-4 py-2.5 text-sm text-[#f4ecd8] focus:border-[#8a641d] focus:outline-none focus:ring-1 focus:ring-[#8a641d]">

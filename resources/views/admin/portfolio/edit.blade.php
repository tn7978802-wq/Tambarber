...
@section('title', 'Sửa tác phẩm - Tâm Barbershop Admin')
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-6">
            <div>
                ...
                <p class="text-xs text-[#f4ecd8]/70">
                    Cập nhật thông tin chi tiết tác phẩm: <span class="text-[#f2d788] font-semibold">{{ $item->title }}</span>
                </p>
            </div>
    ...
    <div class="rounded-[4px] border border-[#3c2c15] bg-[#110d07] p-6 sm:p-8 shadow-2xl space-y-6">
        ...
        <form action="{{ route('admin.portfolio.update', $item) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                ...
                <div class="space-y-2">
                    ...
                    <label for="image_after"
                        ...
                        <span class="absolute inset-0 bg-[#070503]/80 opacity-0 group-hover:opacity-100 flex items-center justify-center text-[#f2d788] text-xs font-bold uppercase tracking-wider transition-all backdrop-blur-sm">
                            <i class="fa-solid fa-arrows-rotate mr-2 text-[#a8342f]"></i> Đổi ảnh mới
                        </span>
                    </label>
                ...
                <div class="space-y-2">
                    ...
                    <label for="image_before"
                        ...
                        @else
                            ...
                            <span class="absolute inset-0 bg-[#070503]/80 opacity-0 group-hover:opacity-100 flex items-center justify-center text-[#f2d788] text-xs font-bold uppercase tracking-wider transition-all backdrop-blur-sm">
                                <i class="fa-solid fa-plus mr-2 text-[#a8342f]"></i> Tải ảnh
                            </span>
                        @endif
                    </label>
                ...
                <div class="space-y-1.5">
                    ...
                    <select name="category" required
                        ...
                        @foreach($categories as $slug => $label)
                            <option value="{{ $slug }}" @selected(old('category', $item->category) === $slug) class="bg-[#070503] text-[#f4ecd8]">
                                {{ $label }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="space-y-1.5">
                    ...
                    <select name="barber_id"
                        ...
                        @foreach($barbers as $barber)
                            <option value="{{ $barber->id }}" @selected((string) old('barber_id', $item->barber_id) === (string) $barber->id) class="bg-[#070503] text-[#f4ecd8]">
                                {{ $barber->name }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="flex items-center gap-3 pt-6 border-t border-[#3c2c15]">
                <button type="submit" 
                        class="rounded-[2px] border border-[#f2d788]/50 bg-gradient-to-r from-[#7c1f22] to-[#8a641d] px-6 py-2.5 text-xs font-bold uppercase tracking-wider text-[#f2d788] shadow-[0_0_15px_rgba(168,52,47,0.4)] hover:brightness-125 transition-all flex items-center gap-2">

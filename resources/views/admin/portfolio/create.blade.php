...
@section('title', 'Thêm tác phẩm mới')
    ...
    <div class="mb-8 border-b border-[#3c2c15] pb-5 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            ...
            <p class="text-xs text-[#f4ecd8]/70 mt-1">
                Thêm hình ảnh vào Thư viện tác phẩm (Portfolio) hiển thị ở trang công khai
            </p>
        </div>
        <div>
                ...
                <div>
                    ...
                    <select name="category" required
                        ...
                        @foreach($categories as $slug => $label)
                            <option value="{{ $slug }}" @selected(old('category') === $slug) class="bg-[#0b0805] text-[#f4ecd8]">
                                {{ $label }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select name="barber_id"
                        ...
                        @foreach($barbers as $barber)
                            <option value="{{ $barber->id }}" @selected((string) old('barber_id') === (string) $barber->id) class="bg-[#0b0805] text-[#f4ecd8]">
                                {{ $barber->name }}
                            </option>
                        @endforeach
                    </select>
